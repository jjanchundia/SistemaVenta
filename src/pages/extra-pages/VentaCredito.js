...
const VentaCredito = () => {
    ...
    const onSuggestionsFetchRequested = ({ value }) => {
        const api = fetch('http://localhost:5158/api/VentaCredito/Productos/' + value)
            .then((response) => {
    ...
    const onSuggestionsFetchRequestedCliente = ({ value }) => {
        const api = fetch('http://localhost:5158/api/VentaCredito/Clientes/' + value)
            .then((response) => {
                return response.ok ? response.json() : Promise.reject(response);
            })
            .then((dataJson) => {
    ...
    const terminarVentaCredito = () => {
        ...
        const api = fetch('http://localhost:5158/api/VentaCredito/Registrar', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json;charset=utf-8'
            },
            body: JSON.stringify(VentaCredito)
        })
            .then((response) => {
                return response.ok ? response.json() : Promise.reject(response);
            })
                ...
                <Col sm={4}>
                            ...
                            <Card>
                                ...
                                <CardBody>
                                    ...
                                    <Row className="mb-2">
                                        <Col sm={12}>
                                            <InputGroup size="sm">
                                                <InputGroupText>N° Meses Total:</InputGroupText>
                                                <Input
                                                    bsSize="sm"
                                                    name="cantidadMeses"
                                                    maxLength="2"
                    ...
                    <Row>
                        <Col sm={12}>
                            <Card>
                                <CardBody>
                                    <Button color="success" block onClick={terminarVentaCredito}>
                                        <i className="fas fa-money-check"></i> Terminar Venta a Crédito
                                    </Button>
                                </CardBody>
                            </Card>
                        </Col>
