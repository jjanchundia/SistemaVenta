...
const Cotizacion = () => {
    ...
    const onSuggestionsFetchRequested = ({ value }) => {
        const api = fetch('http://localhost:5158/api/cotizacion/Productos/' + value)
            .then((response) => {
    ...
    const onSuggestionsFetchRequestedCliente = ({ value }) => {
        const api = fetch('http://localhost:5158/api/cotizacion/Clientes/' + value)
            .then((response) => {
                return response.ok ? response.json() : Promise.reject(response);
            })
            .then((dataJson) => {
    ...
    const terminarcotizacion = () => {
        ...
        const api = fetch('http://localhost:5158/api/cotizacion/Registrar', {
            ...
            .then((dataJson) => {
                reestablecer();
                var data = dataJson;
                Swal.fire('cotizacion Creada!', 'Numero de cotizacion : ' + data.numeroDocumento, 'success');
            })
            .catch((error) => {
                Swal.fire('Opps!', 'No se pudo crear la cotizacion', 'error');
                console.log('No se pudo enviar la cotizacion ', error);
            });
    };
                ...
                <Col sm={4}>
                    ...
                    <Row>
                        <Col sm={12}>
                            <Card>
                                <CardBody>
                                    <Button color="success" block onClick={terminarcotizacion}>
                                        <i className="fas fa-money-check"></i> Terminar cotizacion
                                    </Button>
                                </CardBody>
                            </Card>
                        </Col>
