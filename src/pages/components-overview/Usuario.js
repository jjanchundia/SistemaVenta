...
const Usuario = () => {
    ...
    const obtenerUsuarios = async () => {
        let response = await fetch('http://localhost:5158/api/usuario/Lista');
    ...
    const guardarCambios = async () => {
        ...
        if (usuario.idUsuario == 0) {
            response = await fetch('http://localhost:5158/api/usuario/Guardar', {
                method: 'POST',
                headers: {
    ...
    const eliminarUsuario = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const response = fetch('http://localhost:5158/api/usuario/Eliminar/' + id, { method: 'DELETE' }).then((response) => {
                    if (response.ok) {
                        obtenerUsuarios();
